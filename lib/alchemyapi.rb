...
class AlchemyAPI
	...
	def initialize()
		...
		begin
			...
			if key.length != 40
				...
				puts 'It appears that the key is invalid. Please make sure the file only includes the API key, and it is the correct one.'
				Process.exit(1)
			end
	...
	# The response, already converted from JSON to a Ruby object.
 	#
	#monitor usage with curl -i http://access.alchemyapi.com/calls/info/GetAPIKeyInfo?apikey=<api_key>
